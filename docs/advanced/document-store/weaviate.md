...
You can sort results by any primitive property, typically a text, string, number, or int property. When aquery has a 
natural order (e.g. because of a near<Media> vector search), adding a sort operator will override the order.
...
Consider Documents with the column 'price' and on the return we want to sort these documents by highest price to lowest 
price. We can create an example with the following code:
...
the DocArray/Weaviate uses the NearVector search argument since Weaviate is only being used in this combination to store
vectors generated by DocArray. Sometimes we want to set the certainty at a certain level to limit the return results.
We can do this with the `query_params` argument in the `find()` method.
...
Using the above minimum example we can add the minimum certainty score.
...
For starters, lets assume we want to know when the document was inserted and last updated in the DB. 
