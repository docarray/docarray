...
Then the search with the proposed filter can be implemented and used with the following code. Also, it is noted, that for Elasticsearch, the distance scores can be accessed in the Document's `.scores` dictionary under the key `'score'`.
