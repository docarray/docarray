...
* **We measure vector DB interface**: We benchmark function calls from docarray, not just the underlying backend vector database. Therefore, results for a particular backend can be influenced (positively or negatively) by our interface. 
* **We use similar underlying search algorithms but different implementations**: In this benchmark we focus on setting only parameters `ef`, `ef_construct` and `max_connections` from HNSW. Note that there are other parameters that storage backends can fix than can have a big impact on performance and might even be not easily accesible by users.
