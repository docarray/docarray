...
from tests.doc_index.mongodb.fixture import start_storage  # noqa: F401
from tests.doc_index.mongodb.fixture import DeepNestedDoc, FlatDoc, NestedDoc, SimpleDoc
...
def test_index_simple_schema(ten_simple_docs, use_docarray):
    store = MongoDocumentIndex[SimpleDoc]()
    if use_docarray:
...
def test_index_flat_schema(ten_flat_docs, use_docarray):
    store = MongoDocumentIndex[FlatDoc]()
    if use_docarray:
...
def test_index_nested_schema(ten_nested_docs, use_docarray):
    store = MongoDocumentIndex[NestedDoc]()
    if use_docarray:
...
def test_index_deep_nested_schema(ten_deep_nested_docs, use_docarray):
    store = MongoDocumentIndex[DeepNestedDoc]()
    if use_docarray:
