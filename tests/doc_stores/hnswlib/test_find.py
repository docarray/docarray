...
def test_find_simple_schema(tmp_path, space):
    ...
    store = HnswDocumentStore[SimpleSchema](work_dir=str(tmp_path))
    ...
    docs, scores = store.find(query, embedding_field='tens', limit=5)
...
def test_find_flat_schema(tmp_path, space):
    ...
    store = HnswDocumentStore[FlatSchema](work_dir=str(tmp_path))
    ...
    # find on tens_one
    docs, scores = store.find(query, embedding_field='tens_one', limit=5)
    assert len(docs) == 5
    assert len(scores) == 5
    assert docs[0].id == index_docs[-1].id
    ...
    # find on tens_two
    docs, scores = store.find(query, embedding_field='tens_two', limit=5)
    assert len(docs) == 5
    assert len(scores) == 5
...
def test_find_nested_schema(tmp_path, space):
    ...
    store = HnswDocumentStore[DeepNestedDoc](work_dir=str(tmp_path))
    ...
    docs, scores = store.find(query, embedding_field='d__tens', limit=5)
    assert len(docs) == 5
    assert len(scores) == 5
    assert docs[0].id == index_docs[-2].id
    ...
    docs, scores = store.find(query, embedding_field='d__d__tens', limit=5)
    assert len(docs) == 5
    assert len(scores) == 5
