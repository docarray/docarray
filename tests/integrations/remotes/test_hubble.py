...
def get_test_da(n: int):
    return DocumentArray[TextDoc](TextDoc(text=f'text {i}') for i in range(n))
...
def test_pushpull():
    ...
    da2 = DocumentArray[TextDoc].pull('jinaai://test-da', show_progress=True)
...
def test_pushpull_stream():
    ...
    da2 = DocumentArray[TextDoc].pull_stream(
        'jinaai://test-da-stream', show_progress=True
    )
