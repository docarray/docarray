...
class SequenceLikeMixin(BaseSequenceLikeMixin):
    ...
    def __len__(self):
        """Return the length of :class:`DocumentArray` that uses Redisd as storage

        :return: the length of this :class:`DocumentArrayRedis` object
        """
        try:
            return self._client.llen(self._offset2id_key)
        except:
            return 0
    ...
    def __del__(self):
        ...
        # TODO whether to delete everything
    ...
    def extend(self, docs: Iterable['Document']):
        for doc in docs:
            self._set_doc_by_id(doc.id, doc)
