...
T = TypeVar('T', bound='MongoDocumentIndex')

MONGO_PY_VEC_TYPES = [list, tuple, np.ndarray]
MONGO_PY_TYPES = [bool, int, float, str, docarray.typing.ID]
if torch_imported:
    import torch

    MONGO_PY_VEC_TYPES.append(torch.Tensor)
...
class MongoDocumentIndex(BaseDocumentIndex, Generic[TSchema]):
    def __init__(self, db_config=None, **kwargs):
        super().__init__(db_config=db_config, **kwargs)
        self._db_config = cast(MongoDocumentIndex.DBConfig, self._db_config)
    ...
    def python_type_to_db_type(self, python_type: Type) -> Any:
        ...
        for allowed_type in MONGO_PY_VEC_TYPES:
            if issubclass(python_type, allowed_type):
                return np.ndarray

        if python_type in MONGO_PY_TYPES:
            return python_type
